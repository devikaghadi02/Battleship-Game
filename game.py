""""
---------BATTLESHIPS---------
Pre-reqs:Loops,Strings,Arrrays ,2D Arrays, Global Variables, Methods
How it will work:
...
3.You can choose a row and column as A3 to indicate where to shoot
4.For every shot that hits or misses it will show up in the grid
5.A ship cannot be placed diagonically , so if a shot hits the rest of the ship is in one of 4 directions , left, right,up and down
6.If all ships are unearthed before using up all bullets ,you win else you loose

Legend:
1."." = water or empty space
2."O" = part of ship
3."X" = part of ship that was hit with bullet
4."#" = water that was shot with bullet, a miss because because it hit no ship
"""
...
def accept_valid_bullet_placement():
    """"
    Will accept a valid row and column to place bullet shot.
    Has Return row,col, both are integers.
    """
    global alphabet
    global grid

    is_valid_placeement = False
    row = -1
    col = -1
    while is_valid_placeement is False:
        ...
        if not row.isalpha() or not col.isnumeric():
            print("Error : Please enter a valid row and column such as A3")
            continue
        ...
        if grid[row][col] == "#" or grid[row][col] == "X":
            print("You already have a bullet here, pick somewhere else")
            continue
        if grid[row][col] == "." or grid[row][col] == "O":
            is_valid_placeement = True
